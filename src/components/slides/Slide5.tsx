...
const Slide5 = () => {
  ...
  return (
    <div style={styles.root}>
      {/* Backdrop */}
      <motion.div aria-hidden initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 1 }} style={styles.backdrop} />
      ...
      <div style={styles.row}>
        {/* LEFT: explainer */}
        <motion.div variants={containerVariants} initial="hidden" animate="show" style={styles.leftPanel}>
          <div>
            ...
            <motion.ul variants={containerVariants} initial="hidden" animate="show" style={styles.leftList}>
          ...
          <motion.div initial={miniTrain.initial} animate={miniTrain.animate} transition={miniTrain.transition} style={styles.miniTrainWrap}>
            <svg width="100%" height="110" viewBox="0 0 300 110" preserveAspectRatio="xMidYMid meet">
              ...
              />
              <text x="10" y="105" fontSize="12" fill="#7f8c8d">Epochs</text>
              <text x="250" y="36" fontSize="12" fill="#7f8c8d">Loss ↓</text>
            </svg>
          </motion.div>

          {/* Legend */}
          <motion.div initial={legendIntro.initial} animate={legendIntro.animate} transition={legendIntro.transition} style={styles.legendWrap}>
          ...
          <svg width="100%" height="100%" viewBox="0 0 520 400" preserveAspectRatio="xMidYMid meet">
            ...
            {[
              ...inputLayer.flatMap((inputNeuron, i) =>
                hiddenLayer1.map((hiddenNeuron, j) => ({
                  key: `c-in-${i}-h-${j}`,
                  x1: inputNeuron.x, y1: inputNeuron.y, x2: hiddenNeuron.x, y2: hiddenNeuron.y,
                  delay: 0.45 + i * 0.05,
                }))
              ),
              ...hiddenLayer1.flatMap((hiddenNeuron, i) =>
                outputLayer.map((outputNeuron, j) => ({
                  key: `c-h-${i}-out-${j}`,
                  x1: hiddenNeuron.x, y1: hiddenNeuron.y, x2: outputNeuron.x, y2: outputNeuron.y,
                  delay: 0.9 + i * 0.05,
                }))
              ),
            ].map(({ key, x1, y1, x2, y2, delay }) => (
              <Connection key={key} x1={x1} y1={y1} x2={x2} y2={y2} delay={delay} />
            ))}
            ...
            {[
              ...hiddenLayer1.flatMap((hiddenNeuron, i) =>
                inputLayer.map((inputNeuron, j) => ({
                  key: `glow-h-${i}-in-${j}`,
                  x1: hiddenNeuron.x, y1: hiddenNeuron.y, x2: inputNeuron.x, y2: inputNeuron.y,
              ...
              ...outputLayer.flatMap((outputNeuron, i) =>
                hiddenLayer1.map((hiddenNeuron, j) => ({
                  key: `glow-out-${i}-h-${j}`,
                  x1: outputNeuron.x, y1: outputNeuron.y, x2: hiddenNeuron.x, y2: hiddenNeuron.y,
                  delay: 0.9 + i * 0.2 + j * 0.08,
                }))
              ),
            ].map(({ key, x1, y1, x2, y2, delay }) => (
              <GlowConnection key={key} x1={x1} y1={y1} x2={x2} y2={y2} delay={delay} />
            ))}
            ...
            {[
              ...inputLayer.flatMap((inputNeuron, i) =>
                hiddenLayer1.map((hiddenNeuron, j) => ({
                  key: `d-in-${i}-h-${j}`,
                  startX: inputNeuron.x, startY: inputNeuron.y, endX: hiddenNeuron.x, endY: hiddenNeuron.y,
                  delay: 0.4 + i * 0.15,
                }))
              ),
              ...hiddenLayer1.flatMap((hiddenNeuron, i) =>
                outputLayer.map((outputNeuron, j) => ({
                  key: `d-h-${i}-out-${j}`,
                  startX: hiddenNeuron.x, startY: hiddenNeuron.y, endX: outputNeuron.x, endY: outputNeuron.y,
                  delay: 0.8 + i * 0.15,
                }))
              ),
            ].map(({ key, startX, startY, endX, endY, delay }) => (
              <DataDot key={key} startX={startX} startY={startY} endX={endX} endY={endY} delay={delay} />
            ))}
            ...
            {[
              ...
              <BackpropDot key={key} startX={startX} startY={startY} endX={endX} endY={endY} delay={delay} />
            ))}
            ...
            {hiddenLayer1.map((n, i) => (
              <g key={`hidden1-${i}`}>
                ...
                <HeatPulse x={n.x} y={n.y} base="#93c5fd" highlight="#3b82f6" delay={0.6 + i * 0.75} />
            ...
            {outputLayer.map((n, i) => (
              <g key={`output-${i}`}>
                ...
                <HeatPulse x={n.x} y={n.y} base="#86efac" highlight="#10b981" delay={1.0} />
                <motion.circle
                  cx={n.x}
                  cy={n.y}
                  r={20}
            ...
            <motion.text
              x="30" y="360" fontSize={11} fill="#7f8c8d"
