...
import { motion, AnimatePresence, useInView, cubicBezier, type Variants } from "framer-motion";
import { slideContainer, titleStyle } from "../../styles/slideStyles";

// Gentle easings (use EasingFunction to satisfy TS)
const EASE_SOFT = cubicBezier(0.2, 0.65, 0.3, 0.9);
const EASE_OUT = cubicBezier(0.16, 1, 0.3, 1);
const LINEAR = (t: number) => t; // replace string "linear"

// Story beats (unchanged)
type Beat = {
  id: number;
  step: number;
  title: string;
...
function useAutoplay(length: number, delayMs = 2500) {
  ...
  const [paused, setPaused] = React.useState(true); // start paused by default
  ...
  return { index, setIndex, /* paused, */ setPaused, reset, start, pause, restart };
}

const Slide14: React.FC = () => {
  const rootRef = React.useRef<HTMLDivElement | null>(null);
  const inView = useInView(rootRef, { amount: 0.6, margin: "0px 0px -10% 0px" }); // visible when ~60% on screen
  ...
  // Start ONLY when slide enters the viewport; pause/reset when it leaves
  React.useEffect(() => {
    if (inView) {
      restart(); // begin sequence from step 0
    } else {
      reset(); // go back to start
      setPaused(true); // stop ticking while off-screen
    }
  ...
  return (
    <motion.div
      ...
      onMouseLeave={() => inView && setPaused(false)} // only resume if still visible
    >
      {/* Title */}
      <motion.h2
      ...
      >
        {BEATS.map((b, i) => {
          const isActive = i === index && inView; // highlight only when playing & visible
          return (
            <motion.div
              key={b.id}
